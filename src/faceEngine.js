...
export async function validateFace(dataUrl) {
  ...
  if (w < 200 || h < 200) {
    return { valid: false, step: 'resolution', message: 'Image too small. Please use a photo of at least 512×512 pixels.' };
  }

  // ── Step 2: Blur detection ──
  const blurScore = measureBlur(ctx, w, h);
  if (blurScore < 40) {
    return { valid: false, step: 'quality', message: 'Photo is too blurry. Please use a sharper, well-focused image.' };
  }
  ...
  if (yaw > 18) {
    return { valid: false, step: 'pose', message: 'Please look directly at the camera. Side profiles are not accepted.' };
  }
