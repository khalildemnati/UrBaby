...
function SmartUpload({ who, label, state, onState, onClear }) {
  ...
  const { photo, valid, phase, stepLabel, errMsg, landmarks } = state;
  ...
  async function handleFile(e) {
    ...
    // Step 2: ALWAYS run server-side check (Claude Vision) — catches screens, products, etc.
    // This runs even if MediaPipe passed — it is the authoritative check
    let serverMeta = {};
    if (result.valid || result.step === "mediapipe_unavailable") {
      onState(prev => ({ ...prev, stepLabel: "Running AI face check..." }));
      try {
        ...
        if (!sdata.valid) {
          // Server rejected — this overrides client pass
          result = { valid: false, step: "detection", message: sdata.reason || "Photo did not pass verification." };
        } else {
          result = { valid: true, step: "done", message: "Face Verified" };
          serverMeta = {
        ...
        // Server unreachable — FAIL CLOSED (reject to be safe)
        result = { valid: false, step: "detection", message: "Verification service unavailable. Please try again." };
      }
    }
    ...
    if (result.valid) {
      ...
      onState({
        ...
        errMsg: result.message || "Photo did not pass verification. Please try a different photo.",
        landmarks: result.landmarks || null,
        meta: {},
      });
    }
...
/* ════════════════════════════════════════════ */
const EMPTY_UPLOAD = { photo:null, valid:false, phase:null, stepLabel:"", errMsg:"", landmarks:null, normW:400, normH:400, meta:{} };
