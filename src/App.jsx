...
@media(max-width:540px){
  ...
  .upload-row{grid-template-columns:1fr 22px 1fr!important}
  .gender-row button{padding:10px 4px!important}
...
function SmartUpload({ who, label, state, onState, onClear }) {
  ...
  async function handleFile(e) {
    ...
    // Run MediaPipe validation
    let result = await validateFace(dataUrl);

    // If MediaPipe unavailable, fall back to server
    if (result.step === "mediapipe_unavailable") {
      ...
      try {
        ...
        result = data.valid
          ? { valid: true, step: "done", message: "Face verified" }
          : { valid: false, step: "detection", message: data.reason || "Validation failed." };
      } catch (_) {
        result = { valid: true, step: "done", message: "Face verified" }; // fail open
      }
    }
    ...
    if (result.valid) {
      ...
      onState({
        ...
        valid: false,
        phase: "error",
        stepLabel: STEP_LABELS[result.step]?.text || "Validation failed",
          ...
          <>
            ...
            {isSuccess && (
              <div style={{ position:"absolute", top:0, left:0, right:0, background:C.greenB, padding:"8px 12px", display:"flex", alignItems:"center", gap:7, animation:"slideDown .3s ease" }}>
                <CheckIcon size={12}/>
                <span style={{ fontSize:10, color:"white", letterSpacing:".07em", textTransform:"uppercase", fontWeight:600 }}>Face Verified</span>
              </div>
            )}
            {isError && (
...
/* ════════════════════════════════════════════ */
const EMPTY_UPLOAD = { photo:null, valid:false, phase:null, stepLabel:"", errMsg:"", landmarks:null, normW:400, normH:400 };
  ...
  async function generate() {
    ...
    try {
      ...
      await new Promise((res) => {
        const img = new Image();
        img.onload = res;
        img.onerror = () => res(); // resolve anyway
        img.src = data.imgUrl;
        setTimeout(res, 38000);
      });
